...
class data_grid extends component
{
    ...
    {        
        ...
        env::$page->add_script(OSY_WEB_ROOT.'/js/component/odatagrid.js');
		...
		$this->__check_and_build_filter__();
    }
    
	private function __load_column_object__(){
	...
	private function __check_and_build_filter__(){
        if (!empty($_REQUEST[$this->id.'_filter']) && is_array($_REQUEST[$this->id.'_filter'])){            
            foreach($_REQUEST[$this->id.'_filter'] as $k => $filter){
                if (is_array($filter) && count($filter) == 3){
                    ...
                    $this->add_filter($filter[0],$filter[2],$filter[1]);
                }
            }
        }
		if (empty($_REQUEST[$this->id]) || !is_array($_REQUEST[$this->id])) return;
		foreach($_REQUEST[$this->id] as $field => $value){
			$this->add_filter($field,$value);
		}
    ...
    protected function __build_extra__()
    {
        ...
        if ($this->get_par('filter-show')) $this->__build_filter__ ();
        ...
        $this->add(new hidden_box($this->id.'_order'))->att('class','req-reinit');		
    ...
    private function __build_add__()
	{
    ...
    private function __build_filter__()
    {        
        $cols = array();
        ...
        if (!empty($_REQUEST[$this->id.'_filter']))
        {
            ...
            foreach($_REQUEST[$this->id.'_filter'] as $k => $rec){
                ...
                $lbl->add(new hidden_box($this->id.'_filter['.$k.'][0]'));
                $lbl->add(new hidden_box($this->id.'_filter['.$k.'][1]'));
                $lbl->add(new hidden_box($this->id.'_filter['.$k.'][2]'));
   ...
   private function __build_row(&$grd,$row,$lev=null,$pos=null,$ico_arr=null)
   {
		...
		{        		            			
            ...
            if (!empty($opt['cell']['format'])){
                list($opt,$lev,$pos,$ico_arr) = $this->__format_cell_value__($opt,$pk,$lev,$pos,$ico_arr);
                //var_dump($opt['row']);
            }           
   ...
   private function __format_cell_value__($opt,$pk,$lev,$pos,$ico_arr=null){       
   ...
   private function __data_load__()
   {
        ...
        try {
			...
			$this->__par['rec_num'] = $this->__db->exec_unique($sql_cnt);
		...
		if ($sql_stat = $this->get_par('datasource-sql-stat')){
			try {                			                      
				$sql_stat = env::replacevariable(str_replace('<[datasource-sql]>',$sql,$sql_stat).$whr);				
				$stat = $this->__db->exec_unique($sql_stat,null,'ASSOC');
        ...
        //Scorro il recordset
        $this->__dat = $this->__db->fetch_all($rs);

        //Libero memoria annullando il recordset
        $this->__db->free_rs($rs);
   }
   ...
   private function __data_pivot__($tr){       
       $data = array();
       $hcol = array();
       $hrow = array();
       $fcol = null;
   ...
   private function __set_ext_form__()
   {
        ...
        $res = env::$dbo->exec_query("SELECT frm.o_id    AS id,
